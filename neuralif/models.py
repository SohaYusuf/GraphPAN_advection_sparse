...
class MP_BLOCK(nn.Module):
    ...
    def forward(self, l_x, u_x, l_edge_index, u_edge_index, l_edge_attr, u_edge_attr, global_features):
        ...
        u_edge_embedding, u_node_embeddings, global_features = self.l2(u_x, u_edge_index, u_edge_attr, g=global_features)
...
class NeuralIF(nn.Module):
    ...
    def forward(self, data):
        ...
        L = self.transform_output_matrix(l_node_embedding, l_index, l_edge_embedding, name='L')
        U = self.transform_output_matrix(u_node_embedding, u_index, u_edge_embedding, name='U')
    ...
    def transform_output_matrix(self, node_x, edge_index, edge_values, name=None):
        # force diagonal to be positive
        diag = edge_index[0] == edge_index[1]
        
        
        # tmp1 = 1 / edge_values[diag].squeeze()
        # n = node_x.size()[0]
        # tmp2 = torch.sparse_coo_tensor((range(n), range(n)), tmp1, size=(node_x.size()[0], node_x.size()[0]))
        # tmp2 = tmp2

        if name=='L':
            #t = torch.sparse.mm(t,tmp2)
            edge_values[diag] = 1.0
        # elif name=='U':
        #     if edge_values[diag]<=-1e-10:
        #         edge_values[diag] = -1e-10
        #     elif edge_values[diag]<=1e-10:
        #         edge_values[diag] = 1e-10
        elif name=='U':
