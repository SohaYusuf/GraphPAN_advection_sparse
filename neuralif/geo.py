...
def loss_chol1(L, U, A, sparse=True):
    dev = L.device
...
def loss_chol(L, U, A, sparse=True):
    ...
    n = A.shape[0]
    w = torch.randn(n, 1)
    w = (w / torch.norm(w)).to(dev)
    e = torch.randn(n, 1)
    e = (e / torch.norm(e)).to(dev)
    # * Cholesky decomposition style loss
    # if L.requires_grad:
    #     assert not sparse, "Gradient computation not supported for sparse matrices"
    ...
    if sparse:
        ...
        # https://github.com/rusty1s/pytorch_sparse/issues/45
        # L = L.to_sparse_csr() # convert to sparse CSR format for performance (note: does not support gradients)
        P = torch.sparse.mm(L, U)
        term1 = torch.sparse.mm(w.T, torch.sparse.mm(A, e))
        # print(term1)
        term2  = torch.sparse.mm(w.T, torch.sparse.mm(P, e))
        # print(term2)
        loss = torch.abs(term1 - term2)
        # print(loss)
        return loss
    else:
        A = (A.to(dev)).to_dense().squeeze()
        L = (L.to(dev)).to_dense().squeeze()
        U = (U.to(dev)).to_dense().squeeze()
        loss = torch.abs(w.T @ A @ e - w.T @ (L @ U) @ e)
        return loss
...
def main(config):
    ...
    if config["dataset"] == "random":
        ...
        train_data = FolderDataset(path, config["n"], graph=True)
        validation_data = FolderDataset(path, config["n"], graph=True)
        test_data = FolderDataset(path, config["n"], graph=True, size=test_samples)
    ...
    for epoch in range(config["num_epochs"]):
        ...
        for it, data in enumerate(train_loader):
            ...
            if (it + 1) % 500 == 0:
                try:
                    ...
                    #val_loss_list1.append(np.mean(val_loss))
        ...
        #val_loss_list.append(np.mean(val_loss_list1))
        # log to results
        ...
        #########################################################################
        train_loss_list.append(1/len(train_loader) * running_loss)
        #########################################################################
        if (epoch + 1) % 10 == 0:
            try:
    ...
    plt.figure(1)
    plt.plot(train_loss_list, label='Training Loss', linestyle='--', marker='o')
    plt.xlabel('Epochs')
    ...
    plt.figure(2)
    plt.plot(val_loss_list, label='Validation Loss', linestyle='-', marker='x')
    plt.xlabel('Epochs')
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f'{folder}/val_loss.png')
    plt.show()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f'{folder}/train_val_loss.png')
    plt.show()
